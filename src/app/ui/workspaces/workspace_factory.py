...
def trading_workspace(vm: TradingViewModel) -> QWidget:
    """Trading workspace: strategy builder only.

    Alerts/warnings/recommendations already have a home in the Position
    Monitor dock (see MainWindow._build_docks) -- showing them a second
    time here just ate space the payoff chart needed more."""
    return StrategyBuilderView(vm)
