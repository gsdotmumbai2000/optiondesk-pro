"""Live market data subscription service."""
...
from app.market_data.validation.subscription_validator import SubscriptionValidator
...
class SubscriptionService:
    """Manage pending watchlist and active broker subscriptions."""
    ...
    def __init__(
        ...
        *,
        ...
        validator: SubscriptionValidator | None = None,
    ) -> None:
        """Initialize subscription service."""
        self._broker = broker
        self._event_bus = event_bus
        self._can_subscribe = can_subscribe or (lambda: False)
        self._validator = validator or SubscriptionValidator()
    ...
    def bulk_unsubscribe(
        ...
        *,
        ...
        for symbol, exchange in items:
            self.unsubscribe(symbol, exchange, product_type=product_type)

    def register_defaults(self) -> None:
        """Register default index symbols as pending."""
        self.bulk_subscribe(
            ((symbol, exchange) for symbol, exchange, _ in DEFAULT_INDICES),
    ...
    def activate_pending(self) -> None:
        ...
        for key in list(self._pending):
            self._activate_key(key)
        logger.info("Activated {count} pending market subscriptions", count=len(self._active))
